...
//{#pragma region Files
  ...
  void hud_check_saves(char *save_name) {
    ...
    char * dir = "spectrum";//strrchr(save_name, '.');
    ...
    while(1) {
      ...
      */
      if (gamepad.values[ODROID_INPUT_DOWN]) {
        ...
        hud_options();
        usleep(200000);
        //hud_debounce(ODROID_INPUT_DOWN);
      }
      /*
        LEFT
      */
