...
//{#pragma region Display
  ...
  void hud_options() {
    ...
    for(n = 0; n < 4; n++) {
      ...
      y+=20;
      for(int r = 0; r < 5; r++){for(int c = 0; c < 5; c++) {
        buffer[i] = icons[r+STATE.offset][c] == WHITE ? OPTION == (n+OPTIONS-4) ? GUI.hl : GUI.fg : GUI.bg;i++;
      }}
      ili9341_write_frame_rectangleLE(x, y, w, h, buffer);
      hud_text(x+10,y,STATE.label,false,OPTION == (n+OPTIONS-4)?true:false);
      i = 0;
    } 

    int LIMIT = OPTIONS - 4;
    if(OPTION < LIMIT) {
      STATE = OPTIONS == 9 ? WITHSAVE[OPTION] : WITHOUTSAVE[OPTION];
    } else {
